...
v2.2.1 (08/29/21) => Added example sprite atlas. Fixed pre-rendering of
                     animated sprites.

v2.2   (04/18/21) => Added sprite atlas support. Improved UI.Image support.
                     Fixed legacy particle shader path for Unity 2018+ versions.
                     Additional performance improvements. See [Changelog] for
                     full details.
...
* IMPROVED:
   -> UI.Image support (images can now use non-centered anchors/pivots and
      non-native image sizes)
   -> outlines are pre-rendered once, then displayed as a particle (same as
      manually exporting and enabling the "Use Exported Frame" option)
   -> materials are now reused instead of one unique material per outline

* FIXED:
   -> legacy particle shader path for Unity 2018+ versions
